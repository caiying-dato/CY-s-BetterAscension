...
> 本模组从「能编译」到「10 条进阶全部在游戏里生效」，前后三轮实机排查。
> 下面这些问题的共同点是：**编译期完全看不出来，而且运行时也不报错**。
>
> 记录格式统一为：**现象 → 日志证据 → 根因 → 正解 → 通用教训**。
> 目的不是让你记住答案，而是让你**认出这类形状**。
>
> **最终结果：A11~A20 全部实测生效。**
...
| 5 | **诊断刷屏** | 日志被同一个补丁刷爆 | 挂在超热 getter 上，却每次都打日志 |
| 6 | **内存态与存档不同步** | 功能正常，但**存档/读档后失效** | 幂等标记活在内存里，被标记的效果活在存档里 → 标记比效果活得久 |

> 第 6 类最隐蔽：**功能本身完全正常**，只在「存档 → 退出 → 读档」这条路径上坏掉。
